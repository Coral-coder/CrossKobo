...
# Something stable that needs no account. The point here is the chain -
# DNS, TLS, a verified certificate - not any particular catalogue: the first
# URL tried was an OPDS feed that answers 401 without one, which failed this
# check while proving the fetcher worked.
URL="${CROSSKOBO_FETCH_CHECK_URL:-https://example.com/}"
...
# Any answer at all means the chain worked; 2xx is what this URL gives, and
# an auth or rate-limit answer from some other one still proves DNS, the
# handshake and the certificate.
case "${status}" in
    2*|3*|401|403|429) ;;
    000) echo "no answer at all" >&2; exit 1 ;;
